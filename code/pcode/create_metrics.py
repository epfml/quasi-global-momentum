...
class Metrics(object):
    ...
    def _infer(self):
        ...
        elif self.task == "bert_downstream_tasks":
            return self._bert_finetuning
        else:
    ...
    def _transformer_nmt(self, loss, output, target, **kwargs):
        ...
        try:
            ...
            print("overflow/underflow: {}".format(e))
            return [float("inf"), n_correct / kwargs["n_samples"]]

    def _bert_finetuning(self, loss, output, target, task_metrics):
        eval_res = []
        for task_metric in task_metrics:
            eval_res.append(eval(task_metric)(output, target))
            print(f"[INFO]: gold distribution on validation: {Counter(target)}")
        return eval_res
