...
class TranslationDataset(data.Dataset):
    ...
    def __init__(self, path, exts, fields, **kwargs):
        ...
        example_path = path + "_examples"

        if os.path.exists(example_path):
            print(f"loading data={example_path} from pickle file.")
            examples = op_files.load_pickle(example_path)
        else:
            print(f"loaded data from {path} and saving them to pickle file.")
            examples = []
            ...
            op_files.write_pickle(examples, example_path)
    ...
    def splits(
        ...
        **kwargs,
        ...
        train_data = (
            None
            if train is None
            else cls(os.path.join(path, train), exts, fields, **kwargs)
        )
        val_data = (
            None
            if validation is None
            else cls(os.path.join(path, validation), exts, fields, **kwargs)
        )
        test_data = (
            None
            if test is None
            else cls(os.path.join(path, test), exts, fields, **kwargs)
        )
