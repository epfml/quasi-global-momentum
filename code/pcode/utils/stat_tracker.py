...
class RuntimeTracker(object):
    ...
    def __init__(self, metrics_to_track=["top1"], on_cuda=True):
        self.primary_metric = (
            metrics_to_track[0] if len(metrics_to_track) > 0 else "loss"
        )
    ...
    def _global_evaluate(self, metric):
        return global_average(
            self.stat[metric].sum, self.stat[metric].count, on_cuda=self.on_cuda
        ).item()

    def globally_average_things(self):
        return dict(
            (metric, self._global_evaluate(metric)) for metric in self.things_to_track
        )

    def get_metrics_performance(self):
        return dict((metric, self.stat[metric].avg) for metric in self.things_to_track)
