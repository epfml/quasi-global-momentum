...
def display_training_stat(conf, scheduler, tracker, n_bits_to_transmit):
    ...
    conf.logger.log_metric(
        ...
        tags={"split": "train"},
        display=True and conf.display_info,
    )


def display_general_stat(conf, scheduler, tracker_dict, label, split):
    current_time = time.strftime("%Y-%m-%d %H:%M:%S")
...
def dispaly_best_test_stat(conf, scheduler):
    ...
    #     },
    #     tags={"split": "test", "type": "best_avg_perf_of_local_model"},
    #     display=False,
    # )

    conf.logger.log(
        "best averaged performance of local models at local index {} \
