...
"""original implementation of D2 (it does not include the momentum)."""
        ...
        with kargs["timer"]("sync.apply_grads", epoch=self.conf.epoch_):
            ...
            if self.conf.local_index == 0:
                ...
                flatten_params.buffer = (
                    2 * flatten_params.buffer
                    - self.prev_flatten_params
                    - self.param_groups[0]["lr"]
                    * (flatten_grads.buffer - self.prev_flatten_grads)
                )
            self.prev_flatten_params = prev_flatten_params
