...
class Scheduler(object):
    ...
    def init_learning_rate(self):
        ...
        self.is_warmuped = True if self.is_scaledup and self.conf.lr_warmup else False
    ...
    def init_lr_scheduler(self):
        if self.conf.lr_scheduler == "MultiStepLR":
            if self.conf.lr_milestones is not None:
                ...
                milestones = [
                    int(milestone_ratio * self.conf.num_epochs + 0.5)
    ...
    def update_from_checkpoint(self, checkpoint):
        ...
        self._update_training_progress()
        if self.is_warmuped:
            self.lr_scheduler = self.lr_scheduler.after_scheduler
        ...
        if self.last_epoch > self.total_epoch:
            if self.after_scheduler:
                if not self.finished:
                    ...
                    self.finished = True
                # self.after_scheduler._last_lr is still at the initial value.
                return [x * self.multiplier for x in self.after_scheduler.get_last_lr()]
